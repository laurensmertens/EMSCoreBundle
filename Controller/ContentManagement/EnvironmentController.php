        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                ...
                if (count($anotherObject) != 0) {
                    ...
                    $singleTypeIndex = $this->getParameter('ems_core.single_type_index');
                    if (!$singleTypeIndex) {
                    ...
                    foreach ($this->getContentTypeService()->getAll() as $contentType) {
                        ...
                        if ($this->singleTypeIndex) {
                            $indexName = $this->getContentTypeService()->getNewIndexName($environment, $contentType);
                            $indexes[] = $indexName;
                            $this->getElasticsearch()->indices()->create([
                                'index' => $indexName,
                    ...
                    if ($singleTypeIndex) {
                        ...
                        if(!empty($params ['body']['actions'])) {
                            $this->getElasticsearch()->indices()->updateAliases($params);
                        }
